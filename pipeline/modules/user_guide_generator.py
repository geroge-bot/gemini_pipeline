...
2. [机位参数标签]
   ...
   - 机位角度变化：仰拍、平拍、俯拍、侧面、正面切换、低角度仰拍、将镜头放低、抬高机位、斜上方俯拍、切换为侧面视角、由正面转为侧面、贴近地面拍摄、水平平视拍摄等
   - 语气种类：建议型 - 可以、尝试、试试、不妨...；陈述型 - 将、把、（或直接描述，例如后退合适距离使用长焦虚化背景）
...
4. [高频词汇替代]
   - 对于特别高频容易使用的词语，以50% 的概率进行随机的替换，使用其他的描述，例如：
     - "平视": ["水平拍摄", "手机垂直"],
     - "尝试": ["试试", "可以", "不妨"],
     - "用长焦": ["切换更高焦段", "拉长焦段", "放大焦段"],
     - "放低": ["降低手机", "降低镜头", "稍微低一点", "放低视角", "从低角度拍摄"],
     - "开启长焦": ["切换更高焦段", "拉长焦段", "放大焦段"],
     - "展现": ["突出", "拍出", "塑造", "表现"]  
...
3. 整体引导 (27-35字)：
   ...
   - 结构（使用逗号连接，句号结尾）：
     ...
     ii. 机位与参数：参考[机位参数标签]方向自行发挥，注意，如果画面出现虚化，需要使用具体描述进行引导。语气从以下示例中随机参考选择。（约10字）
       - 示例：尝试放低视角，将镜头下移并开启长焦，可以平视靠近并利用长焦弱化后景，尝试采用九十度顶拍，拉开距离使用长焦模糊背景，保持手机端平垂直俯拍
       - 注意不要提示“人像模式”
       -【绝对封锁线：严禁描述虚化】**：**严禁在核心视觉建议中出现任何与“虚化”、“背景虚化”、“柔焦”、“大光圈”等相关的描述与词汇！**
