...
v2 仍采用本地文件持久化：全局任务列表写入 `state.json`，每个任务的条目写入 `items.json`，阶段记录写入 `records.json`。不依赖数据库。
...
- 若原图最长边大于 1024，会等比缩放到最长边 1024。
- 若原图最长边不超过 1024，直接返回原文件。
- 缩放后的预览会写入预览缓存目录。
- 缓存 key 包含绝对路径、修改时间、文件大小和最大边长度，原图变化后会自动生成新缓存。
...
以下需求用于约束后续迭代和验收。当前实现已经覆盖其中的图片缩放、预热缓存、前端预加载、可视化分页等基础能力；未完全覆盖的指标应作为优化目标。

### 12.1 首屏任务列表

- 首页只应加载任务摘要和必要配置，不应加载完整 `items.json` 图片内容或原图文件。
- 任务列表接口应在本地常见任务规模下保持可感知快速响应；目标为 1 秒内返回任务列表和 summary。
...
- 图片加载应优先请求 1024 预览图，而不是原图。
- 如果预览缓存已存在，当前图片应在用户肉眼感知上接近即时切换；如果缓存不存在，生成预览不应阻塞整页，只影响当前图片响应。
...
- 默认图片接口必须返回预览图，最长边不超过 1024。
- 原图只应在明确请求 `original=1` 时返回。
